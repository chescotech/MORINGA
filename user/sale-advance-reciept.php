                    ...
                    <div class="name_">
                        ...
                        <form method="post" action="entry.php" onSubmit="return proceed()">
                            <table border="0" align="center" width="150000" class="top1" >
                                ...
                                <?php
                                $query = mysqli_query($con, "select advace_id,product.prod_id AS prod_id,prod_name,prod_sell_price,SUM(qty) AS qty,amount,customer_name from advance_payments_tb INNER JOIN product ON product.prod_id=advance_payments_tb.prod_id AND advance_payments_tb.order_no='$orderNumber'  group by prod_name ")or die(mysqli_error($con));
                                $grand = 0;

                                $query2 = mysqli_query($con, "select advace_id,prod_name,prod_sell_price,SUM(qty) AS qty,amount,"
                                        . "customer_name,product.prod_id AS prod_id from advance_payments_tb INNER JOIN "
                                        . "product ON product.prod_id=advance_payments_tb.prod_id AND advance_payments_tb.order_no='$orderNumber' "
                                        . " group by prod_name ")or die(mysqli_error($con));
                                ...
                                while ($row = mysqli_fetch_array($query)) {
                                    ...
                                    $prod_id = $row['prod_id'];
                                    $advace_id = $row['advace_id'];

                                    $dateis = date('Y-m-d');

                                    //mysqli_query($con, "INSERT INTO sales_details(prod_id,qty,price,sales_id,order_no) VALUES('$prod_id','$qty','$price','$sales_id','$orderNumber')")or die(mysqli_error($con));

                                    mysqli_query($con, "update product set prod_qty=prod_qty-'$qty' where prod_id='$prod_id'")or die(mysqli_error($con));

                                    mysqli_query($con, "update advance_payments_tb set status='Sold' where advace_id='$advace_id'")or die(mysqli_error($con));
                                ...
                                <td class="box" width="" style=" color: black"><b><h2><u></u>
                                           ...
                                           Total Paid K ' . number_format($advanceAmount, 2) . '        
                                           <br>.........................................................................................</br>    
                                        </h2></b></td>
                            </tr>';
                                ?>
