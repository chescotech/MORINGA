...
if ($barcode != "") {
    ...
    if ($countRows > 0) {
        ...
        if ($qty <= $noQtyInStock) {
            ...
            $query1 = mysqli_query($con, "select * from advance_payments_tb where prod_id='$prod_id' and branch_id='$branch' AND order_no='0' ")or die(mysqli_error($con));
            $count = mysqli_num_rows($query1);

            $total = $price * $qty;

            if ($count > 0) {
                mysqli_query($con, "update advance_payments_tb set qty=qty+'$qty',price=price+'$total' where prod_id='$prod_id' and branch_id='$branch' AND order_no='0' ")or die(mysqli_error($con));
            } else {
                mysqli_query($con, "INSERT INTO advance_payments_tb(prod_id,qty,price,branch_id,user_id) VALUES('$prod_id','$qty','$price','$branch','$user_id')")or die(mysqli_error($con));
            }
            echo "<script>document.location='advance-sale.php?cid=$cid'</script>";
        } else {
    ...
    if ($qty <= $noQtyInStock) {
        ...
        $query1 = mysqli_query($con, "select * from advance_payments_tb where prod_id='$name' and branch_id='$branch' AND order_no='0' ")or die(mysqli_error($con));
        $count = mysqli_num_rows($query1);

        $total = $price * $qty;

        if ($count > 0) {
            mysqli_query($con, "update advance_payments_tb set qty=qty+'$qty',price=price+'$total' where prod_id='$name' and branch_id='$branch' AND order_no='0' ")or die(mysqli_error($con));
        } else {
