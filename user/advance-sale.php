	      ...
	      <div class="col-md-9">
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title">Advance Sale</h3>
                </div>
                <div class="box-body">
                  <!-- Date range -->
                  <form method="post" action="add-advance-payment.php">
							...
							<label for="date">Product Name</label>
								...
								<?php
								  ...
								  include('../dist/includes/dbcon.php');
									 $query2=mysqli_query($con,"select * from product where branch_id='$branch' AND stock_branch_id='$branch_id_user' AND prod_qty >0 AND DATE(NOW()) < DATE(expire_date) OR expire_date ='' order by prod_name")or die(mysqli_error($con));
...
<?php
		$user_id = $_SESSION['id'];
		$query=mysqli_query($con,"select * from advance_payments_tb INNER join product On product.prod_id = advance_payments_tb.prod_id AND advance_payments_tb.order_no='0' GROUP BY product.prod_id ")or die(mysqli_error($con));
			$grand=0;
		while($row=mysqli_fetch_array($query)){
				$id=$row['advace_id'];
				$total= $row['prod_sell_price'] * $row['qty'];
                                $unitPrice = $row['prod_sell_price'] / $row['qty'];
				$grand=$grand+$total;
...
?>
                      <tr >
                        ...
                        <td>
							...
							<a href="#updateordinance<?php echo $row['advace_id'];?>" data-target="#updateordinance<?php echo $row['advace_id'];?>" data-toggle="modal" style="color:#fff;" class="small-box-footer"><i class="glyphicon glyphicon-edit text-blue"></i></a>

              <a href="#delete<?php echo $row['temp_trans_id'];?>" data-target="#delete<?php echo $row['advace_id'];?>" data-toggle="modal" style="color:#fff;" class="small-box-footer"><i class="glyphicon glyphicon-trash text-red"></i></a>
              
						</td>
                      </tr>
					  <div id="updateordinance<?php echo $row['advace_id'];?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
	<div class="modal-dialog">
	  <div class="modal-content" style="height:auto">
			  ...
			  <form class="form-horizontal" method="post" action="update-advance-qty.php" enctype='multipart/form-data'>
					<input type="hidden" class="form-control" name="cid" value="<?php echo $cid;?>" required>  	
					<input type="hidden" class="form-control" id="price" name="id" value="<?php echo $row['advace_id'];?>" required> 
                                        <input type="hidden" class="form-control" id="price" name="prod_id" value="<?php echo $row['prod_id'];?>" required> 
				<div class="form-group">
 ...
 <!--end of modal-->  
<div id="delete<?php echo $row['advace_id'];?>" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
  <div class="modal-dialog">
    <div class="modal-content" style="height:auto">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              ...
              <div class="modal-body">
        <form class="form-horizontal" method="post" action="delete-advance.php" enctype='multipart/form-data'>
          <input type="hidden" class="form-control" name="cid" value="<?php echo $cid;?>" required>   
          <input type="hidden" class="form-control" id="price" name="id" value="<?php echo $row['advace_id'];?>" required>  
...
<?php }?>					  
            ...
            <div class="col-md-3">
              <div class="box box-primary">
                ...
                <div class="box-body">
                  <!-- Date range -->
          <form method="post" name="autoSumForm" action="advance-sales-add.php">
				  <div class="row">
					 <div class="col-md-12">
                ...
                <label for="date">Advance Amount</label><br>
                <input type="text" style="text-align:right" class="form-control" onFocus="startCalc();"   onBlur="stopCalc();"  id="cash" name="amount" placeholder="Advance Amount"  required="" autocomplete="off">
              </div><!-- /.form group -->
						 
              <div class="form-group" id="tendered">
                <label for="date">Customers Name</label><br>
                <input type="text" style="text-align:right" class="form-control" onFocus="startCalc();"   onBlur="stopCalc();"  id="cash" name="customer_name" placeholder="Customers Name"  required="" autocomplete="off">
              </div><!-- /.form group -->
