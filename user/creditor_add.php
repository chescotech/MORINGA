...
include('../dist/includes/dbcon.php');
	...
	$query2=mysqli_query($con,"select * from customer where cust_last='$last' and cust_first='$first' and branch_id='$branch'")or die(mysqli_error($con));
		...
		{	
			...
			mysqli_query($con,"INSERT INTO customer(cust_last,cust_first,cust_address,cust_contact,branch_id,balance,credit_status,bday,nickname,house_status,years,rent,emp_name,emp_no,emp_address,emp_year,occupation,salary,spouse,spouse_no,spouse_emp,spouse_details,spouse_income,comaker,comaker_details,cust_pic,price_tag) 
				VALUES('$last','$first','$address','$contact','$branch','0','pending','$bday','$nickname','$house_status','$years','$rent','$emp_name','$emp_no','$emp_address','$emp_year','$occupation','$salary','$spouse','$spouse_no','$spouse_emp','$spouse_details','$spouse_income','$comaker','$comaker_details','default.gif','$price_tag')")or die(mysqli_error($con));
