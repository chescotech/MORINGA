...
if (mysqli_num_rows($passwordChecker) == 0) {
    ...
    $itemChecker = mysqli_query($con, " select * from temp_trans WHERE temp_trans_id ='$cid' ")
            or die(mysqli_error($con));
    ...
    $result = mysqli_query($con, "UPDATE temp_trans SET amount='$amount', discount_type='$discount_type'  WHERE temp_trans_id ='$cid'") or die(mysqli_error($con));

    echo "<script>document.location='cash_transaction.php'</script>";
    
} else {
      echo "<script type='text/javascript'>alert('Invalid Username or Password!');
      document.location='cash_transaction.php'</script>";
       }
?>