...
$end = $_POST['end'];
 ...
 { 
  $id = $_POST['id'];
  mysqli_query($con,"UPDATE term SET term_start='$start',term_end='$end' where term_id='$id'") or die(mysqli_error($con)); 
  echo "<script type='text/javascript'>alert('Successfully updated term!');</script>";
 } 
  if (isset($_POST['addterm']))
 { 
  mysqli_query($con,"INSERT INTO term(term_start,term_end) VALUES('$start','$end')")or die(mysqli_error($con));  
