                                                ...
                                                <tbody>
                                                    ...
                                                    if (isset($_POST['display'])) {
                                                        ...
                                                        $query = mysqli_query($con, "select * from product natural join supplier natural"
                                                                . " join category  "
                                                                . "INNER JOIN stores_branch ON stores_branch.id=product.stock_branch_id AND expire_date BETWEEN '$startDate' AND '$endDate'  ")or die(mysqli_error($con));

                                                        echo ' <center><h3 class="box-title" style=" color: black"><b><u>Product Expiry Report from ' . $startDate . ' to ' . $endDate . '</u></b></h3></center>';
                                                    } else {
                                                        //$query = mysqli_query($con, "SELECT stock_branch_id,SUM(qty) AS qty,prod_name,prod_desc,name, sales_details.price AS prod_sell_price,sales.date_added  FROM sales_details INNER JOIN sales ON sales.sales_id=sales_details.sales_id INNER JOIN user ON user.user_id = sales.user_id INNER JOIN product ON product.prod_id = sales_details.prod_id AND DATE(sales.date_added) = DATE(NOW()) GROUP BY prod_name,stock_branch_id,sales_details.price")or die(mysqli_error($con));
                                                             $query = mysqli_query($con, "select * from product natural join supplier natural"
                                                                . " join category  "
                                                                . "INNER JOIN stores_branch ON stores_branch.id=product.stock_branch_id WHERE DATE(expire_date) = DATE(NOW())  ")or die(mysqli_error($con));

                                                        
                                                        echo ' <center><h3 class="box-title" style=" color: black"><b><u>Todays Expiry Report.</u></b></h3></center><br>';
                                                    }
