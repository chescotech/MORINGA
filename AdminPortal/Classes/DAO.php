...
class DAO
{
    ...
    public function getProductPrice($prodid, $con, $price_tag, $cust_id)
    {
        $getCustomerDataQuery = mysqli_query($con, "SELECT * FROM customer WHERE cust_id='$cust_id'") or die(mysqli_error($con));
        $InfoRows = mysqli_fetch_array($getCustomerDataQuery);
        $price_tag = $InfoRows['price_tag'];
    ...
    public function addToTransferLogs($con, $ex_prod_id, $qty, $stock_branch_id, $branch_name, $user_id)
    {
        mysqli_query($con, "INSERT INTO stock_trasfers_tb(prod_id,qty,user_id,moved_to)
                                                        VALUES('$ex_prod_id','$qty','$user_id','$stock_branch_id')") or die(mysqli_error($con));
        
        mysqli_query($con, "INSERT INTO stock_audit_tb(prod_id,count,added_to,action)
                                                        VALUES('$ex_prod_id','$qty','$branch_name','Transfer')") or die(mysqli_error($con));
    }
}
