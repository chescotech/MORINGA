...
include('../dist/includes/dbcon.php');
	...
	$query2=mysqli_query($con,"select * from supplier where supplier_id='$id'")or die(mysqli_error($con));
	...
	if ($type=="Payment")
	  {
		$remarks="added a payment of $amount to $supplier_name";  
	   	mysqli_query($con,"UPDATE supplier SET balance=balance-'$amount' where supplier_id='$id'") or die(mysqli_error($con)); 
	...
	else if ($type=="Credit Memo")
	 {
		...
		mysqli_query($con,"UPDATE customer SET balance=balance-'$amount' where supplier_id='$id'") or die(mysqli_error($con)); 
		mysqli_query($con,"INSERT INTO history_log(user_id,action,date) VALUES('$user_id','$remarks','$date')")or die(mysqli_error($con));
	 }
