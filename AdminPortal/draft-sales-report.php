...
<html>
    <head>
        ...
        <title>Credit Sales Report | <?php include('../dist/includes/title.php'); ?></title>
                                ...
                                <div class="box-body">
                                    ...
                                    <table id="example2" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            $query = mysqli_query($con, "SELECT date,customer_name,order_no,SUM(qty) AS totalqty,SUM(price) AS total, SUM(price*qty) AS totalbill,name FROM `draft_temp_trans` INNER JOIN user ON user.user_id=draft_temp_trans.user_id  GROUP BY order_no")or die(mysqli_error($con));
                                            while ($row = mysqli_fetch_array($query)) {
                                                $totalAmountCollected += $row['total'];
                                                $orderNo = $row['order_no'];

                                                $query_ = mysqli_query($con, "SELECT SUM(amount) AS totalpaid FROM `part_payments_tb` INNER JOIN user ON user.user_id=part_payments_tb.user_id AND order_no='$orderNo'  ")or die(mysqli_error($con));
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo number_format($row_['totalpaid'], 2); ?></td> 
                                                    <td><?php echo number_format($row['totalbill'] - $row_['totalpaid'], 2); ?></td> 
                                                    <td>
