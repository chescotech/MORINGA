                                    ...
                                    <form method="post">
                                        ...
                                        <div class="col-lg-2">
                                            ...
                                            <select class="form-control select2" style="width: 100%;" name="branch_id" required >
                                                <option value="all_branches">All Users</option>
                                                <?php
                                                ...
                                                <tbody>
                                                    ...
                                                    if (isset($_POST['display'])) {
                                                        ...
                                                        if ($branch_id != 'all_branches'){                                                           
                                                            $query = mysqli_query($con, " SELECT stock_branch_id,SUM(qty) AS qty,prod_name,prod_desc,name,"
                                                                    . "prod_sell_price,sales.date_added  FROM sales_details INNER JOIN sales ON sales.sales_id=sales_details.sales_id"
                                                                    ...
                                                                    . "AND user.user_id='$userId'  GROUP BY prod_name,stock_branch_id,sales.date_added")or die(mysqli_error($con));
                                                        } else if ($branch_id == 'all_branches'){
                                                            $query = mysqli_query($con, " SELECT stock_branch_id,SUM(qty) AS qty,prod_name,prod_desc,name,"
                                                                    . "prod_sell_price,sales.date_added  FROM sales_details INNER JOIN sales ON sales.sales_id=sales_details.sales_id"
                                                                    . " INNER JOIN user ON user.user_id = sales.user_id INNER JOIN product ON product.prod_id = sales_details.prod_id"
                                                                    . " AND sales.date_added BETWEEN '$startDate' AND '$stop_date'"
                                                                    . "GROUP BY prod_name,stock_branch_id ,sales.date_added ")or die(mysqli_error($con));
                                                        } else {
                                                            $query = mysqli_query($con, " SELECT stock_branch_id,SUM(qty) AS qty,prod_name,prod_desc,name,prod_sell_price,sales.date_added  FROM sales_details INNER JOIN sales ON sales.sales_id=sales_details.sales_id INNER JOIN user ON user.user_id = sales.user_id INNER JOIN product ON product.prod_id = sales_details.prod_id AND sales.date_added BETWEEN '$startDate' AND '$stop_date'  GROUP BY prod_name,stock_branch_id,sales.date_added  ")or die(mysqli_error($con));
                                                        }

                                                        echo ' <center><h3 class="box-title" style=" color: black"><b><u>User Cash Out Sales Report from ' . $start . ' to ' . $end . '</u></b></h3></center>';
                                                    } else {
                                                        $query = mysqli_query($con, "SELECT stock_branch_id,SUM(qty) AS qty,prod_name,prod_desc,name, prod_sell_price,sales.date_added  FROM sales_details INNER JOIN sales ON sales.sales_id=sales_details.sales_id INNER JOIN user ON user.user_id = sales.user_id INNER JOIN product ON product.prod_id = sales_details.prod_id AND DATE(sales.date_added) = DATE(NOW()) GROUP BY prod_name")or die(mysqli_error($con));
                                                        echo ' <center><h3 class="box-title" style=" color: black"><b><u>Todays User Cash Out Sales Report</u></b></h3></center><br>';
                                                    }
