...
<?php
		...
		$query=mysqli_query($con,"select * from term order by term_end desc")or die(mysqli_error($con));
...
<?php
	...
	$settings=mysqli_query($con,"select * from settings natural join term")or die(mysqli_error($con));
		...
		<form method="POST" action="settings_add.php">
		  ...
		  <div class="form-group">
			    ...
			    <option value="<?php echo $rowset['term_id'];?>"><?php echo date("M d, Y",strtotime($rowset['term_start']))." - ".date("M d, Y",strtotime($rowset['term_end']));?></option>
                      <?php
				$query2=mysqli_query($con,"select * from term order by term_end desc")or die(mysqli_error($con));
